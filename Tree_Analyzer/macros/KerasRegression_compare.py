...
def build_model():
  model = keras.Sequential([
    layers.Dense(10, activation=tf.nn.relu, kernel_regularizer=keras.regularizers.l1(0.002), input_shape=[len(train_dataset.keys())]),
    #keras.layers.Dropout(0.2),
    layers.Dense(20, activation=tf.nn.relu, kernel_regularizer=keras.regularizers.l1(0.002)),
...
history = model.fit(
  ...
  epochs=EPOCHS, batch_size=82048,
...
[plot_perf(results,
           ...
           "label[pt]>"+str(i*10)+" & label[pt]<"+str((i+1)*10)) for i in range(0,11)]
