...
inputVariables = ['gen_e',
                  ...
                  #'pf_hcalFrac1', 'pf_hcalFrac2', 'pf_hcalFrac3', 'pf_hcalFrac4', 'pf_hcalFrac5', 'pf_hcalFrac6', 'pf_hcalFrac7'
                  "p", 'pt']
targetVariable = 'gen_e'
### InputFiles
#inputFiles = ["singlePi_histos_trees_new_samples.root","singlePi_histos_trees_valid.root"]
inputFiles = ["singlePi_histos_trees_new_samples.root"] #"singlePi_histos_trees_depth_samples.root"
...
my_cut = barrel_train & (dataset['pf_totalRaw'] >0) & (dataset['gen_e']>0) & (dataset['gen_e']<500)
...
#del compareData['pf_hcalFrac1'], compareData['pf_hcalFrac2'], compareData['pf_hcalFrac3'], compareData['pf_hcalFrac4'], compareData['pf_hcalFrac5'], compareData['pf_hcalFrac6'], compareData['pf_hcalFrac7']
del compareData['p'], compareData['pt']
...
def RunTraining(dataset):
  ...
  def build_model(inputs):
    ...
    layers.Dense(inputs, activation=tf.nn.relu, kernel_regularizer=keras.regularizers.l2(0.000), input_shape=[inputs]),# 4, relu, l1(0)
    ...
    layers.Dense(24, activation=tf.nn.relu, kernel_regularizer=keras.regularizers.l1(0.0000)), #2, relu, l2(0)
    keras.layers.Dropout(0.4), # 0
    layers.Dense(24, activation=tf.nn.relu, kernel_regularizer=keras.regularizers.l1(0.0000)), #2, relu, l2(0)
    keras.layers.Dropout(0.4), # 0
    layers.Dense(1)
    ])
    #optimizer = tf.train.RMSPropOptimizer(0.001)
    optimizer  = keras.optimizers.Adam(lr=.001, decay= .001 / 700) # lr = .0001, decay = .0001 / 200
  ...
  def plot_history(history):
    ...
    plt.show()
    fig.savefig("compare/mean_Drop.pdf")
    plt.clf()
    plt.close()
...
test_predictions, test_labels = RunTraining(dataset)
compare_pred, compare_labels = RunTraining(compareData)
results = pd.DataFrame({'gen_e':test_labels,'DNN':test_predictions})
comp_results = pd.DataFrame({'gen_e':compare_labels, 'DNN':compare_pred})

comp_results['Response'] = (comp_results['DNN'] - comp_results['gen_e'])/comp_results['gen_e']
...
#fig.savefig("compare/model_acc.png")
#plt.show()
#plt.clf()
#plt.close()
...
### compare resonpse ###
res_DNN   = (comp_results['DNN']-comp_results['gen_e'])/comp_results['gen_e']
res_Keras = (test_predictions-test_labels)/test_labels
plot_hist_compare([res_DNN,res_Keras],100,['NoTracks','Tracks'],"(Pred-True)/True [E]","compare/perf_comparison.pdf")
### compare pt distribution ###
plot_hist_compare([comp_results['DNN'],test_predictions],25,['NoTracks','Tracks'],"E","compare/pt_comparison.pdf")
...
### Pred/True vs True ###
profile_plot_compare(comp_results['gen_e'], comp_results['DNN']/comp_results['gen_e'], 'NoTracks',
                     test_labels, test_predictions/test_labels, 'Tracks',
                     100, 0, 500,
                     "True [E]", "Pred/True [E]", "compare/scale_comparison.pdf")
### Response vs True ###
profile_plot_compare(comp_results['gen_e'], (comp_results['DNN']-comp_results['gen_e'])/comp_results['gen_e'], 'NoTracks',
                     test_labels, (test_predictions-test_labels)/test_labels, 'Tracks',
                     100, 0, 500,
                     "True [E]", "(Pred-True)/True [E]", "compare/response_comparison.pdf")
...
, range=np.array([(0,1.5),(0,1.5)]))
  ...
  plt.show()
  fig.savefig("compare/EH_vs_E.pdf")
  plt.clf()
  plt.close()
...
E_bin_response(comp_results,results,20,['NoTracks','Tracks'],-1.2,1.2,"(Pred-True)/True (GeV)","compare/1DResponse.pdf")    
